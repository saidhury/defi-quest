...
const QuestDetail = () => {
    ...
    useEffect(() => {
        const fetchQuestDetails = async () => {
            try {
                // In a real app, this might be a single API call `GET /quests/${questId}`
                // For the hackathon, filtering the full list is fine.
                const response = await axios.get(`${API_URL}/quests`);
                const selectedQuest = response.data.find(q => q.id === parseInt(questId));
    ...
    const getQuestContract = async () => {
        ...
        return new ethers.Contract(QUEST_CONTRACT_ADDRESS, QuestContractAbi, signer);
    };
